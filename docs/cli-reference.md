...
# Run a command — string form (shell): the whole string is passed to sh -c / cmd /C.
# Shell syntax works: pipes, redirects, &&, environment variable expansion.
action:
  type: cmd
  execute: "gh issue comment {{issue_number}} --repo {{repo}} --body 'ready'"

# Run a command — list form (exec): argv is passed directly, no shell involved.
# No quoting issues; each element is a separate argument.
action:
  type: cmd
  execute: [gh, issue, comment, "{{issue_number}}", --repo, "{{repo}}", --body, ready]

# Clone the repo and run a command inside it; no write-back
action:
  type: cmd-checkout
  execute: "./scripts/check-package.sh --version 8.0"
  cwd: "./subdir"  # relative to checkout root; omit to use the repo root
