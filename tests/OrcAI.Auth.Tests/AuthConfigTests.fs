...
let ``getActiveProfile returns correct profile`` () =
    let cfg = A.AuthConfig.withProfiles "my-app" [ "my-app", A.ProfileEntry.app "123" "/key.pem" "install-1" ]
...
let ``getActiveProfile returns error when active key is empty`` () =
    let cfg = A.AuthConfig.withProfiles "" [ "my-app", A.ProfileEntry.app "123" "/key.pem" "install-1" ]
    match getActiveProfile cfg with
...
let ``getActiveProfile returns error when active points to missing profile`` () =
    let cfg = A.AuthConfig.withProfiles "missing" [ "my-app", A.ProfileEntry.app "123" "/key.pem" "install-1" ]
    match getActiveProfile cfg with
...
let ``upsertProfile adds a new profile and sets it as active`` () =
    let cfg    = A.AuthConfig.withProfiles "" []
...
let ``upsertProfile replaces an existing profile`` () =
    let cfg    = A.AuthConfig.withProfiles "my-app" [ "my-app", A.ProfileEntry.app "old-id" "/old.pem" "old-install" ]
...
let ``upsertProfile sets the named profile as active`` () =
    let cfg    = A.AuthConfig.withProfiles "other" [ "other", A.ProfileEntry.pat "tok"; "my-app", A.ProfileEntry.app "id" "/k.pem" "i" ]
...
let ``switchActive succeeds when profile exists`` () =
    let cfg = A.AuthConfig.withProfiles "app1" [ "app1", A.ProfileEntry.app "1" "/k1.pem" "i1"; "app2", A.ProfileEntry.app "2" "/k2.pem" "i2" ]
...
let ``switchActive returns error when profile does not exist`` () =
    let cfg = A.AuthConfig.withProfiles "app1" [ "app1", A.ProfileEntry.app "1" "/k1.pem" "i1" ]
