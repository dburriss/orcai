...
let ``merge: local Some value wins over global Some value`` () =
    let g = { empty with ContinueOnError = Some false; MaxConcurrency = Some 2 }
    let l = { empty with ContinueOnError = Some true;  MaxConcurrency = Some 8 }
    let m = merge g l
    Assert.Equal(Some true, m.ContinueOnError)
    Assert.Equal(Some 8,    m.MaxConcurrency)

[<Fact>]
let ``merge: global Some value used when local is None`` () =
    let g = { empty with ContinueOnError = Some true; DefaultOrg = Some "myorg" }
    let l = empty
    let m = merge g l
    Assert.Equal(Some true,    m.ContinueOnError)
...
let ``resolve: reads and applies global config when only global exists`` () =
    ...
    fs.File.WriteAllText("/home/user/.config/orcai/config.json", """{"continueOnError": true, "maxConcurrency": 8}""")
    let result = resolve (fs :> System.IO.Abstractions.IFileSystem) "/home/user" "/work"
    Assert.Equal(Some true, result.ContinueOnError)
...
let ``resolve: local overrides global on overlapping keys`` () =
    let fs = MockFileSystem()
    fs.Directory.CreateDirectory("/home/user/.config/orcai") |> ignore
    fs.File.WriteAllText("/home/user/.config/orcai/config.json", """{"continueOnError": true, "maxConcurrency": 2}""")
    fs.Directory.CreateDirectory("/work/.orcai") |> ignore
    fs.File.WriteAllText("/work/.orcai/config.json", """{"continueOnError": false, "maxConcurrency": 8}""")
    let result = resolve (fs :> System.IO.Abstractions.IFileSystem) "/home/user" "/work"
    Assert.Equal(Some false, result.ContinueOnError)
    Assert.Equal(Some 8,     result.MaxConcurrency)
