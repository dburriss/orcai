...
- [x] GitHub App authentication is not supported for assigning copilot, so we need to add support for using a PAT for just the copilot assignment step if the user has authenticated with a GitHub App. This would involve allowing users to specify both a GitHub App and a PAT in their config/env, and using the appropriate credentials for each step of the process. We would need to ensure that the PAT is only used for the copilot assignment and not for any other calls if a GitHub App is also configured.
